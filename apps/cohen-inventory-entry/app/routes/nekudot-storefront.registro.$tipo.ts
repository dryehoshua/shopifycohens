...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const sourceUrl = new URL(sourcePath, backend);
  const ibCode = new URL(request.url).searchParams.get("ib");
  if (type === "blue" && ibCode) sourceUrl.searchParams.set("ib", ibCode.slice(0, 40));
  ...
  return proxy.liquid(`
    ...
    <script>
      (function () {
        var frame = document.querySelector('.nk-form-frame');
        if (!frame) return;
        window.addEventListener('message', function (event) {
          if (event.source !== frame.contentWindow || event.origin !== new URL(frame.src).origin) return;
          if (!event.data || event.data.type !== 'cohens-registration-size') return;
          var height = Number(event.data.height);
          if (Number.isFinite(height) && height >= 200 && height <= 12000) frame.style.height = (Math.ceil(height) + 8) + 'px';
        });
      })();
    </script>
  `);
