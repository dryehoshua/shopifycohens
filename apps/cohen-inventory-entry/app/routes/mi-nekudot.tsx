...
export default function MemberPortal() {
  ...
  return <main className="nk-shell"><header className="nk-brand"><span className="nk-mark">C</span><div><strong>Cohen&apos;s · Nekudot</strong><small>Hola, {card.displayName}</small></div></header><div className="nk-grid">
    <section className="nk-panel"><p className="nk-eyebrow">Resumen</p><h1>Tu cuenta</h1><div className="nk-summary"><div className="nk-stat">Saldo disponible<strong>{money(card.availableCents)}</strong></div><div className="nk-stat">Tarjeta<strong>{tierLabel(card.cardTier)}</strong></div></div>
      {card.ownedBroker ? <div className="nk-dual-wallet"><div><small>MIS NEKUDOT PERSONALES</small><strong>{money(card.availableCents)}</strong><span>{tierLabel(card.cardTier)} para compras</span></div><div><small>MI WALLET IB</small><strong>{money(card.ownedBroker.commissionBalanceCents)}</strong><span>Comisiones separadas · código {card.ownedBroker.code}</span></div></div> : null}
      ...
      <div className="nk-status"><h2>{card.cardTier === "GOLDEN" ? "Tu tarjeta Gold" : "Subir a Gold"}</h2><img src="/nekudot/cards/golden.webp" alt="Tarjeta Gold" style={{ width: "100%", maxWidth: 340, borderRadius: 16 }} /><p><strong>$300 MXN al mes = 300 Nekudot para gastar en Cohen's.</strong> Cada Nekudot vale $1 MXN en compras. Además obtienes 8% de cashback.</p><p>Tus Nekudot no caducan mientras Gold esté activa. Al activarla se anulan Plata y Blue; tus puntos y vales se conservan.</p>{card.cardTier !== "GOLDEN" || !card.active ? <a className="nk-button" href="https://cohenskosher.com/apps/nekudot#golden">Subir a Gold</a> : null}</div>
      <h2>Compras en Cohen&apos;s</h2><div className="nk-orders">{orders.length ? orders.map((order) => <article className="nk-order" key={order.id}><div><strong>{order.name}</strong><br /><small>{new Date(order.processedAt).toLocaleDateString("es-MX")} · {order.displayFinancialStatus} · {order.displayFulfillmentStatus}</small>{order.cashbackProcessed ? <span className="nk-order-cashback">+{money(order.clientEarnedCents)} Nekudot{order.brokerEarnedCents ? ` · +${money(order.brokerEarnedCents)} IB` : ""}</span> : <span className="nk-order-pending">Cashback pendiente de confirmación</span>}</div><strong>{new Intl.NumberFormat("es-MX", { style: "currency", currency: order.currentTotalPriceSet.shopMoney.currencyCode }).format(Number(order.currentTotalPriceSet.shopMoney.amount))}</strong></article>) : <p>Aún no hay compras vinculadas a esta cuenta.</p>}</div>
      <h2>Identificadores activos</h2><div className="nk-credential-list">{card.credentials.map((credential) => <span className="nk-pill" key={credential.id}>{credential.kind === "NFC" ? "NFC" : credential.kind === "BARCODE" ? "Código de barras" : credential.kind} · •••• {credential.lastFour}</span>)}</div>
