...
export const meta: MetaFunction = () => [{ title: "Nekudot · Cohen's" }];
...
function cardTone(tier: string) { return tier === "BLUE" ? " blue" : tier === "GOLDEN" ? " golden" : tier === "VOUCHER" ? " vales" : ""; }
...
export default function MemberPortal() {
  ...
  return <main className="nk-shell"><header className="nk-brand"><span className="nk-mark">C</span><div><strong>Cohen&apos;s · Nekudot</strong><small>Hola, {card.displayName}</small></div></header><div className="nk-grid">
    ...
    <aside className={`nk-card${cardTone(card.cardTier)}`}><div className="nk-card-top"><div><p className="nk-eyebrow">Cohen&apos;s</p><div className="nk-tier">{tierLabel(card.cardTier)}</div></div>{card.photoFileName ? <img className="nk-photo" src={`/nekudot-photo/${card.id}`} alt={card.displayName} /> : <div className="nk-photo nk-photo-placeholder">☺</div>}</div><div className="nk-card-person"><h2>{card.displayName}</h2><p>{card.community || "Comunidad Cohen's"}</p><strong className="nk-card-balance">{money(card.availableCents)} disponibles</strong></div><img className="nk-qr" src={card.qrDataUrl} alt="Código QR Nekudot" /><img className="nk-barcode" src={card.barcodeDataUrl} alt="Código de barras Nekudot" /><p className="nk-card-code">ID •••• {card.credentialLastFour}</p><p className="nk-card-note">Presenta el QR o código de barras en los puntos de venta Cohen&apos;s.</p></aside>
  </div></main>;
}
