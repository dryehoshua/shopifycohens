...
export default function BrokerPortal() {
  ...
  const referredClients = dashboard.clients.filter((client) => client.id !== dashboard.ownerMember?.id);
  const activeClients = referredClients.filter((client) => client.active).length;
  return <main className="nk-shell"><header className="nk-brand"><span className="nk-mark">C</span><div><strong>Cohen&apos;s · Portal IB</strong><small>Hola, {dashboard.displayName}</small></div></header><section className="nk-panel">
    ...
    {dashboard.ownerMember ? <div className="nk-dual-wallet"><div><small>MIS NEKUDOT PERSONALES</small><strong>{money(dashboard.ownerMember.balanceCents - dashboard.ownerMember.reservedCents)}</strong><span>Blue · 5% para compras</span></div><div><small>MI WALLET IB</small><strong>{money(dashboard.commissionBalanceCents)}</strong><span>5% de comisión, saldo separado</span></div></div> : <div className="nk-status">Todavía no vinculaste tu tarjeta Blue personal. Regístrate con tu propio código para recibir 5% personal y 5% IB en saldos separados.</div>}
    ...
    <h2>Personas bajo tu IB</h2><div className="nk-table-wrap"><table className="nk-table"><thead><tr><th>Persona</th><th>Comunidad</th><th>Tarjeta</th><th>Nekudot generados</th><th>Estado</th></tr></thead><tbody>{referredClients.length ? referredClients.map((client) => <tr key={client.id}><td><strong>{client.displayName}</strong></td><td>{client.community || "Sin comunidad"}</td><td><span className="nk-pill">{tierLabel(client.cardTier)}</span></td><td>{money(client.lifetimeEarnedCents)}</td><td>{client.active ? "Activo" : "Inactivo"}</td></tr>) : <tr><td colSpan={5}>Todavía no hay personas referidas registradas con tu código.</td></tr>}</tbody></table></div>
