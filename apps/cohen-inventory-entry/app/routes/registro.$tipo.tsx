...
const PAGE_OPTIONS = {
  plata: {
    ...
    socialImage: "/og-nekudot-plata.png",
  ...
  blue: {
    ...
    socialImage: "/og-nekudot-blue.png",
  ...
  golden: {
    ...
    socialImage: "/og-nekudot-golden.png",
  ...
  vales: {
    ...
    socialImage: "/og-nekudot-vales.png",
    ...
    <div className="nk-grid">
      <section className="nk-panel">
        ...
        {result ? <div className="nk-status">{result.status === "PENDING_PAYMENT" ? "Tu cliente fue creado. La membresía Golden quedará activa al autorizar la suscripción mensual." : "Registro completado. Tu tarjeta digital ya está lista."} {result.ibName ? <>Tu IB es <strong>{result.ibName}</strong>. </> : null}{result.checkoutUrl ? <><a className="nk-button" href={result.checkoutUrl}>Activar suscripción Golden</a> </> : null}<Link to="/nekudot">Abrir Nekudot</Link>.</div> : verification ? <div className="nk-existing-verify">
          ...
          {matchReview ? <fieldset className="nk-existing-matches"><legend>¿Eres alguno de estos registros?</legend><p>Ocultamos los datos por seguridad. Si eliges uno, confirmaremos por SMS antes de mostrar sus puntos.</p>{matchReview.matches.map((match) => <label key={match.token}><input type="radio" name="existingMatch" value={match.token} required /><span><strong>{match.name}</strong><small>{match.phone}{match.email ? ` · ${match.email}` : ""}</small></span></label>)}<label><input type="radio" name="existingMatch" value="none" required /><span><strong>No soy ninguno de estos</strong><small>Crear mi tarjeta y continuar a Nekudot.</small></span></label></fieldset> : null}
